...
(
	function(e, t, n)
	{
		...
		t.html5imageupload = function(r, i)
		{
			...
			this.button.done = '<div class="btn btn-success btn-ok" title="' + (this.options.okTitle || "Kaydet") + '"><i class="glyphicon glyphicon-ok"></i></div>';
		...
		t.html5imageupload.prototype = 
		{
			_init: function()
			{
				...
				{	
					//bak burda tnin içini sildik foto gitti ve halen çalışıyor.
					//hayır bi de width height falan atayabiliyomuşuz ya la? bu sayede biz database e istediğimiz çözünürlükte kaydederiz ki :D
					t(i).data("name", r.image).append(t("<img />").addClass("img-rounded").attr("src", r.image).attr("width", "128").attr("height", "128")); //previewi img rounded yaptık ki köşelerden yesin.. 
					//n.readImage(r.image, r.image, r.image, n.imageMimes[r.image.split(".").pop()])
					
					var l = t('<div class="preview tools" style="opacity:0.40;filter:alpha(opacity=40)" onmouseover="this.style.opacity=1;this.filters.alpha.opacity=100" onmouseout="this.style.opacity=0.40;this.filters.alpha.opacity=40"></div>');
			...
			_ajax: function(e)
			{
				...
				t.ajax(
				{
					...
					// HATALI! tip: t(r).data("filename", e.filename), //n.imageMimes[i.split(".").pop()],
					success: function(e)
					{
		...
		//DEĞİŞTİRDİM AMA Bİ DEĞİŞİKLİK YOK KALDIRILABİLİR TEKRAR ÖNEMLİYSE!!!!
							//t(r).data("name", i[0]);
				...
				//if (e.options.onAfterProcessImage) e.options.onAfterResetImage.call(n.imageFinal())
			},
			...
			imageFinal: function()
			{
				...
				var s = t('<div class="tools final" style="opacity:0.40;filter:alpha(opacity=40)" onmouseover="this.style.opacity=1;this.filters.alpha.opacity=100" onmouseout="this.style.opacity=0.40;this.filters.alpha.opacity=40">');
				...
				if (i.download == true)
				{
					var o = t('<div class="download final" style="opacity:0.19;filter:alpha(opacity=19)" onmouseover="this.style.opacity=0.6;this.filters.alpha.opacity=60" onmouseout="this.style.opacity=0.11;this.filters.alpha.opacity=11" />');
			...
			reset: function()
			{
				...
				var i = e.options;
				e.image = null;
				t(n).find(".img-round").remove(); // burayı da preview yerine img-round yaptık.. 
				...
				if (i.removeurl != null)// && !empty(t(n).data("name"))) //BURAYI KAPATINCI ŞEY OLUYO, İŞTE İLK GELDİĞİNDE FOTOSUZ OLSA DA YÜKLEYİNCE TEKRAR SİLİNCE SİLİYO data-image OLMASA DA.. ;)
				{
					t.ajax(
					{
						type: "POST",
						...
						{
							image:  t(n).data("name")
							//var osman: this.options.downloadLabel
						},
						success: function(t)
						{
			...
			_tools: function()
			{
				var n = this;
				var r = n.element;
				var i = t('<div class="tools"style="opacity:0.40;filter:alpha(opacity=40)" onmouseover="this.style.opacity=1;this.filters.alpha.opacity=100" onmouseout="this.style.opacity=0.40;this.filters.alpha.opacity=40"></div>');
